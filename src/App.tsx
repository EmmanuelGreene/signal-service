...
interface Overview {
  ...
  sentimentLabel: string; avgConfidence: number;
...
/* ─── Chat Component ─── */
function ChatSidebar(_props: { signals: Signal[], overview: Overview | null }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: 'AI offline. Check the dashboard signals.' }]);
    ...
    <div className="chat-sidebar">
      ...
      <div className="chat-input-row">
        <input className="chat-input" value={input} onChange={e => setInput(e.target.value)}
          onKeyDown={e => e.key === 'Enter' && send()} placeholder="Ask about a coin..." disabled={loading} />
...
function App() {
  ...
  return (
    <div className="app">
      <div className={`main-area ${showChat ? 'with-chat' : ''}`}>
        ...
        <header className="header">
          <div className="header-left">
            ...
            <div className="subtitle">CoinGecko · RSI · EMA(8/21) · MACD · Bollinger · Paper Trading</div>
          </div>
        ...
        {overview && (
          <div className="overview-bar">
            ...
            <div className="ov-item"><span className="ov-label">Sentiment</span><span className={`ov-val ${overview.sentimentLabel === 'Bullish' ? 'up' : overview.sentimentLabel === 'Bearish' ? 'dn' : ''}`}>{overview.sentimentLabel}</span></div>
            <div className="ov-divider" />
      ...
      {/* CHAT SIDEBAR */}
      {showChat && <ChatSidebar signals={signals} overview={overview} />}
    </div>
  );
}
